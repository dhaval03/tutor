...
class FileManager extends \Opencart\System\Engine\Controller {
	public function index(): void {
		$this->load->language('extension/tmdmultivendor/vendor/filemanager');
		...
		$this->response->setOutput($this->load->view('extension/tmdmultivendor/vendor/filemanager', $data));
	}
	...
	public function list(): void {
		...
		if (isset($this->request->get['directory'])) {
			$directory = $vendorfolder1 . html_entity_decode($this->request->get['directory'], ENT_QUOTES, 'UTF-8') . '/';
		} else {
			...
			foreach ($images as $image) {
				if (substr(str_replace('\\', '/', realpath($image)), 0, strlen($base)) == $base) {
					...
					$data['directories'][] = [
						...
						'type' => 'directory',
						'href' => $this->url->link('extension/tmdmultivendor/vendor/filemanager|list',  '&directory=' . urlencode(Helper\Utf8\substr($image, Helper\Utf8\strlen($base))) . $url)
					];
				}
			}
		}
		...
		$data['parent'] = $this->url->link('extension/tmdmultivendor/vendor/filemanager|list', $url);
		...
		$data['refresh'] = $this->url->link('extension/tmdmultivendor/vendor/filemanager|list', $url);
		...
		$data['pagination'] = $this->load->controller('extension/tmdmultivendor/vendor/pagination', [
			'total' => count(array_merge((array)$directories, (array)$files)),
			'page'  => $page,
			'limit' => 16,
			'url'   => $this->url->link('extension/tmdmultivendor/vendor/filemanager|list', $url . '&page={page}')
		]);

		$this->response->setOutput($this->load->view('extension/tmdmultivendor/vendor/filemanager_list', $data));
	}
	...
	public function upload(): void {
		...
		$vendorfolder=DIR_IMAGE . 'catalog/multivendor/'.$vendor_id;
		/// 09 08 2018 ///
		$vendorfolder1=DIR_IMAGE . 'catalog/multivendor/';
	...
	public function folder(): void {
		...
		// Make sure we have the correct directory
		if (isset($this->request->get['directory'])) {
			$directory = $vendorfolder . html_entity_decode($this->request->get['directory'], ENT_QUOTES, 'UTF-8') . '/';
		} else {
			$directory = $vendorfolder;
		}
	...
	public function delete(): void {
		...
		foreach ($paths as $path) {
			...
			// Check path exists
			if (($path == $vendorfolder) || (substr(str_replace('\\', '/', realpath($base . $path)) . '/', 0, strlen($base)) != $base)) {
