...
// echo "yes";exit;
    ...
    */
    ...
    private function getProductInfo($product, $simpleList = false, $customFields = false)
    {
        ...
        $special_excluding_tax_formated = $this->currency->format(0, $this->currency->getRestCurrencyCode());
        $special_formated = $this->currency->format(0, $this->currency->getRestCurrencyCode());
        $discounts = array();
        $options = array();
            ...
            if(!empty($additional_images)) {
                foreach ($additional_images as $additional_image) {
                    if (isset($additional_image['image']) && !empty($additional_image['image']) && file_exists(DIR_IMAGE . $additional_image['image'])) {
                        $images[] = $this->model_tool_image->resize($additional_image['image'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                        $original_images[] = $this->urlPrefix . 'image/' . $additional_image['image'];
                    } else {
                        $images[] = $this->model_tool_image->resize('no_image.png', $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                ...
                foreach ($option['product_option_value'] as $option_value) {
                    if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        if ((($this->customer->isLogged() && $this->config->get('config_customer_price')) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
                            ...
                            $price_excluding_tax_formated = $this->currency->format(0, $this->currency->getRestCurrencyCode());
                            $price_formated = $this->currency->format(0, $this->currency->getRestCurrencyCode());
                        }
                        if (isset($option_value['image']) && file_exists(DIR_IMAGE . $option_value['image'])) {
                            $option_image = $this->model_tool_image->resize($option_value['image'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                        } else {
                            $option_image = $this->model_tool_image->resize('no_image.png', $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                        }
        ...
        $retval = array(
            ...
            'viewed' => isset($product['viewed'])?$product['viewed']:'',
...
//            );
            ...
            foreach ($categories as $category) {
                ...
                if (isset($categoryDetails['category_id'])) {
                    ...
                    if (isset($categoryDetails['image']) && !empty($categoryDetails['image']) && file_exists(DIR_IMAGE . $categoryDetails['image'])) {
                        $image = $this->model_tool_image->resize($categoryDetails['image'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
    ...
    private function getManufacturerInfo($manufacturer)
    {
        ...
        if (isset($manufacturer['image']) && !empty($manufacturer['image']) && file_exists(DIR_IMAGE . $manufacturer['image'])) {
            $image = $this->model_tool_image->resize($manufacturer['image'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                ...
                switch ($setting['key']) {
                    ...
                    case 'config_image':
                        ...
                        if (!empty($setting['value']) && is_file(DIR_IMAGE . $setting['value'])) {
                            $this->json['data']['thumb'] = $this->model_tool_image->resize($setting['value'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                            $this->json['data']['store_image'] = $this->urlPrefix . 'image/' . $setting['value'];
                        } else {
                            $this->json['data']['thumb'] = $this->model_tool_image->resize('no_image.png', $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                    ...
                    foreach ($all as $product_info) {
                        ...
                        if ($product_info) {
                            if ($product_info['image']) {
                                $image = $this->model_tool_image->resize($product_info['image'], $this->config->get('config_shoppingcartrestapi_image_width'), $this->config->get('config_shoppingcartrestapi_image_height'));
                            } else {
                                $image = false;
                            }
