...
class Import extends \Opencart\System\Engine\Model {
		...
		public function category($category,$parent_id,$store_id,$language_id)
		{
			...
			$category_id = $this->db->getLastId();
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . $language_id . "', name = '" . $this->db->escape((string)$category) . "', meta_keyword = '" . $this->db->escape((string)$category) . "', meta_description = '" . $this->db->escape((string)$category) . "', description = '" . $this->db->escape((string)$category) . "'");
			...
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" .$this->db->escape((string)str_replace("'",'',$category)). "' and store_id='".$store_id."' and language_id='".$language_id."'");
			if(!$query->row)
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape((string)str_replace("'",'',$category)) . "',store_id='".$store_id."',language_id='".$language_id."'");
			}
			else
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape((string)$category) .'-'.$category_id."',store_id='".$store_id."',language_id='".$language_id."'");
			}
			return $category_id;
			}
		...
		public function barnd($barnd,$store_id,$language_id,$vendor_id)
		{
		$query = $this->db->query("SELECT  m.manufacturer_id FROM " . DB_PREFIX . "manufacturer m left join " . DB_PREFIX . "vendor_to_manufacturer vtm on vtm.manufacturer_id=m.manufacturer_id left join " . DB_PREFIX . "manufacturer_to_store mts on mts.manufacturer_id=m.manufacturer_id WHERE m.name = '" .  $this->db->escape((string)$barnd)."' and mts.store_id='".(int)$store_id."' and vtm.vendor_id='".(int)$vendor_id."'");
		
		if(isset($query->row['manufacturer_id'])) {
		return $query->row['manufacturer_id'];			
		} else {
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer SET name = '" . $this->db->escape((string)$barnd) . "', sort_order = '0'");
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" .$this->clean($barnd). "' and store_id='".(int)$store_id."' and language_id='".$language_id ."'");
		if(!isset($query->row))
			{
				$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'manufacturer_id=" . (int) $manufacturer_id . "', keyword = '" . $this->clean($barnd) . "',store_id='".$store_id."',language_id='".$language_id."'");
			
			}
			else
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'manufacturer_id=" . (int)$manufacturer_id . "', keyword = '" . $this->clean($barnd) .'-'.(int)$manufacturer_id."',store_id='".(int)$store_id."', language_id='".$language_id ."'");
			}
	...
	public function option($data,$language_id,$vendor_id)
	{
		...
		$query=$this->db->query("select * from  " . DB_PREFIX . "option_description od  left join " . DB_PREFIX . "vendor_to_option vto on vto.option_id=od.option_id where  od.name = '" . $this->db->escape((string)$optionname) . "' and vto.vendor_id='".(int)$vendor_id."' limit 0,1");
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "language");
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape((string)$optionname) . "'");
	...
	{	
		...
		$query=$this->db->query("select  * from  " . DB_PREFIX . "option_value_description where option_id = '" . (int)$option_id . "' and name = '" . $this->db->escape((string)$optionvaluename) . "'");
		if($query->row)
		{
		...
		else
		{
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', sort_order = '".$sort_order."'");
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape((string)$optionvaluename) . "'");
	...
	{
		$query = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p left join `" . DB_PREFIX . "vendor_to_product` vt on vt.product_id=p.product_id  where p.product_id='".(int)$product_id."' ");
		if(isset($query->row['product_id'])) {
			return $query->row['product_id'];
		}
	}
	...
	public function addproduct($data,$language_id,$extra,$vendor_id)
	{
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id. "', name = '" . $this->db->escape((string)$data['name']) . "', meta_keyword = '" . $this->db->escape((string)$data['meta_keyword']) . "', meta_description = '" . $this->db->escape((string)$data['meta_description']) . "', description = '" . $this->db->escape((string)$data['description']) . "', tag = '" . $this->db->escape((string)$data['tag']) . "',meta_title = '" . $this->db->escape((string)$data['meta_title']) . "'");
	...
	$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "'");
		...
		if (isset($data['product_special'])) {
			foreach ($data['product_special'] as $product_special) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', priority = '" . (int)$product_special['priority'] . "', price = '" . (float)$product_special['price'] . "', date_start = '" . $this->db->escape((string)$product_special['date_start']) . "', date_end = '" . $this->db->escape((string)$product_special['date_end']) . "'");
			}
		}
		...
		if (isset($data['product_category'])) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE `key` = 'product_id=" . (int)$product_id. "' and language_id='".$language_id."'");
		
		if ($data['keyword']) {
			$data['keyword']=$this->clean($data['keyword']);
			if (isset($data['product_store'])) {
			foreach ($data['product_store'] as $store_id) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'product_id=" . (int) $product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "',store_id='".(int)$store_id."',language_id='".(int)$language_id."'");
			}
			}
		}
	...
	public function editproduct($data,$product_id,$language_id,$extra,$vendor_id)
	{
		...
		if(isset($query->row['product_id']))
		{
				$this->db->query("update   " . DB_PREFIX . "product SET model = '" . $this->db->escape((string)$data['model']) . "', sku = '" . $this->db->escape((string)$data['sku']) . "', upc = '" . $this->db->escape((string)$data['upc']) . "', ean = '" . $this->db->escape((string)$data['ean']) . "', jan = '" . $this->db->escape((string)(string)$data['jan']) . "', isbn = '" . $this->db->escape((string)$data['isbn']) . "', mpn = '" . $this->db->escape((string)$data['mpn']) . "', location = '" . $this->db->escape((string)$data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '".$data['available']."', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '".$data['point']."', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '".$data['tax_class_id']."', sort_order = '" . (int)$data['sort_order'] . "' WHERE product_id = '" . (int)$product_id . "'");
		}

		
		else
		{
			$this->db->query("insert " . DB_PREFIX . "product SET model = '" . $this->db->escape((string)$data['model']) . "', sku = '" . $this->db->escape((string)$data['sku']) . "', upc = '" . $this->db->escape((string)$data['upc']) . "', ean = '" . $this->db->escape((string)$data['ean']) . "', jan = '" . $this->db->escape((string)$data['jan']) . "', isbn = '" . $this->db->escape((string)$data['isbn']) . "', mpn = '" . $this->db->escape((string)$data['mpn']) . "', location = '" . $this->db->escape((string)$data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '".$data['available']."', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '".$data['point']."', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '".$data['tax_class_id']."', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(),product_id = '" . (int)$product_id . "'");
		}
	...
	$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "'");
		...
		if ($data['keyword']) {
			...
			if (isset($data['product_store'])) {
			foreach ($data['product_store'] as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET `key` = 'product_id=" . (int) $product_id . "', keyword = '" . $this->db->escape((string)$data['keyword']) . "',store_id='".$store_id."',language_id='".$language_id."'");
		...
		if(isset($data['reviews']))
		{
			...
			foreach($data['reviews'] as $review)
			{
				$this->db->query("insert " . DB_PREFIX . "review set product_id = '" . (int)$product_id . "',customer_id='".(int)$review['customer_id']."',author='".$this->db->escape((string)$review['author'])."',text='".$this->db->escape((string)$review['text'])."',rating='".$review['rating']."',status='".$review['status']."',date_added='".$review['date_added']."',date_modified='".$review['date_modified']."'");
			}
		...
		if(isset($extra))
		{
			foreach($extra as $key=>$value)
			{
				$this->db->query("update " . DB_PREFIX . "product set `".$key."`='".$this->db->escape((string)$value)."' WHERE product_id = '" . (int)$product_id . "'");
			}
		}
	}
	...
	public function filtergroup($filtergroup,$language_id,$vendor_id)
	{
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_group_description where name='".$this->db->escape((string)$data[0])."' and language_id='".(int)$language_id."'");
			if(!$query->row) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group` SET sort_order = '" . (int)$data[1] . "'");
			$filter_group_id = $this->db->getLastId();
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "language");
				
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description SET filter_group_id = '" . (int)$filter_group_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape((string)$data[0]) . "'");
	...
	public function filtername($filtername,$language_id,$vendor_id)
	{
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_group_description  where name='".$this->db->escape((string)$datafull[0])."' and language_id='".(int)$language_id."'");
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_description  fgd   left join " . DB_PREFIX . "vendor_to_option vto fgd.filter_id=fgd.filter_id where fgd.name='".$this->db->escape((string)$data[0])."' and fgd.filter_group_id = '" . (int)$filter_group_id . "' and vto.vendor_id='".(int)$vendor_id."'");
		if(!isset($query->row)) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$data[1] . "'");
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$language_id . "', filter_group_id = '" . (int)$filter_group_id . "', name = '" . $this->db->escape((string)$data[0]) . "'");
	...
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute_group_description agd  left join " . DB_PREFIX . "vendor_to_attribute_group vto on agd.attribute_group_id=agd.attribute_group_id where agd.name='".$this->db->escape((string)$groupname)."' and agd.language_id = '" . (int)$language_id . "' and vto.vendor_id='".(int)$vendor_id."'");
			if($query->row) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_group_description SET attribute_group_id = '" . (int)$attribute_group_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape((string)$groupname) . "'");
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "attribute a LEFT JOIN ".DB_PREFIX."attribute_description ad on(a.attribute_id = ad.attribute_id) left join " . DB_PREFIX . "vendor_to_attribute_group vto on a.attribute_group_id=a.attribute_group_id where ad.name='".$this->db->escape((string)$attname)."' and ad.language_id = '" . (int)$language_id . "' and vto.vendor_id='".(int)$vendor_id."'");
			if($query->row) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_description SET  language_id = '" . (int)$language_id . "',attribute_id='".(int)$attribute_id."', name = '" . $this->db->escape((string)$attname) . "'");
	...
	public function getVendors($data) {
		...
		if (isset($data['filter_status'])){
		 	$sql .=" and status like '".$this->db->escape((string)$data['filter_status'])."%'";
		}
		
		if (isset($data['filter_date'])){
		 	$sql .=" and date_added like '".$this->db->escape((string)$data['filter_date'])."%'";
		}
