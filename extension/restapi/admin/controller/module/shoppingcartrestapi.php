<?php
namespace Opencart\Admin\Controller\Extension\Restapi\Module;
use \Opencart\System\Helper AS Helper;
error_reporting(0);
class ShoppingCartRestApi extends \Opencart\System\Engine\Controller {

    public function index() {
        $this->load->language('extension/restapi/module/shoppingcartrestapi');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $apiOrderId = "1234567";//$this->request->post['module_shoppingcartrestapi_order_id'];
            if(!empty($apiOrderId) && strlen($apiOrderId) > 5 ) {

                    $this->request->post['module_shoppingcartrestapi_licensed_on'] = $apiOrderId;
                /*if (isset($_POST['nJvNVJoMHcQVIuHk']) && !empty($_POST['nJvNVJoMHcQVIuHk'])) {
                    $this->request->post['module_shoppingcartrestapi_licensed_on'] = $_POST['nJvNVJoMHcQVIuHk'];
                } else {
                    $this->request->post['module_shoppingcartrestapi_status'] = 0;
                }*/

                $this->model_setting_setting->editSetting('module_shoppingcartrestapi', $this->request->post);
                $this->model_setting_setting->setOauthClient($this->request->post['module_shoppingcartrestapi_client_id'], $this->request->post['module_shoppingcartrestapi_client_secret']);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/restapi/module/shoppingcartrestapi', 'user_token=' . $this->session->data['user_token'], true)
        );

        if (isset($this->request->post['module_shoppingcartrestapi_status'])) {
            $data['module_shoppingcartrestapi_status'] = $this->request->post['module_shoppingcartrestapi_status'];
        } else {
            $data['module_shoppingcartrestapi_status'] = $this->config->get('module_shoppingcartrestapi_status');
        }

        if (isset($this->request->post['module_shoppingcartrestapi_key'])) {
            $data['module_shoppingcartrestapi_key'] = $this->request->post['module_shoppingcartrestapi_key'];
        } else {
            $data['module_shoppingcartrestapi_key'] = $this->config->get('module_shoppingcartrestapi_key');
        }

        if (isset($this->request->post['module_shoppingcartrestapi_order_id'])) {
            $data['module_shoppingcartrestapi_order_id'] = $this->request->post['module_shoppingcartrestapi_order_id'];
        } else {
            $data['module_shoppingcartrestapi_order_id'] = $this->config->get('module_shoppingcartrestapi_order_id');
        }
        if (isset($this->request->post['module_shoppingcartrestapi_client_id'])) {
            $data['module_shoppingcartrestapi_client_id'] = $this->request->post['module_shoppingcartrestapi_client_id'];
        } else {
            $data['module_shoppingcartrestapi_client_id'] = $this->config->get('module_shoppingcartrestapi_client_id');
        }

        if (isset($this->request->post['module_shoppingcartrestapi_client_secret'])) {
            $data['module_shoppingcartrestapi_client_secret'] = $this->request->post['module_shoppingcartrestapi_client_secret'];
        } else {
            $data['module_shoppingcartrestapi_client_secret'] = $this->config->get('module_shoppingcartrestapi_client_secret');
        }

        if (isset($this->request->post['module_shoppingcartrestapi_token_ttl'])) {
            $data['module_shoppingcartrestapi_token_ttl'] = $this->request->post['module_shoppingcartrestapi_token_ttl'];
        } else {
            $data['module_shoppingcartrestapi_token_ttl'] = $this->config->get('module_shoppingcartrestapi_token_ttl');
        }

        if (!isset($data['module_shoppingcartrestapi_token_ttl']) || empty($data['module_shoppingcartrestapi_token_ttl'])) {
            $data['module_shoppingcartrestapi_token_ttl'] = 2628000;
        }

        if (isset($this->request->post['module_shoppingcartrestapi_allowed_ip'])) {
            $data['module_shoppingcartrestapi_allowed_ip'] = $this->request->post['module_shoppingcartrestapi_allowed_ip'];
        } else {
            $data['module_shoppingcartrestapi_allowed_ip'] = $this->config->get('module_shoppingcartrestapi_allowed_ip');
        }

        if (isset($this->request->post['module_shoppingcartrestapi_enable_logging'])) {
            $data['module_shoppingcartrestapi_enable_logging'] = $this->request->post['module_shoppingcartrestapi_enable_logging'];
        } else {
            $data['module_shoppingcartrestapi_enable_logging'] = $this->config->get('module_shoppingcartrestapi_enable_logging');
        }

        //set default client id
        if (!isset($data['module_shoppingcartrestapi_client_id']) || empty($data['module_shoppingcartrestapi_client_id'])) {
            $data['module_shoppingcartrestapi_client_id'] = 'shopping_oauth_client';
        }

        //set default client secret
        if (!isset($data['module_shoppingcartrestapi_client_secret']) || empty($data['module_shoppingcartrestapi_client_secret'])) {
            $data['module_shoppingcartrestapi_client_secret'] = 'shopping_oauth_secret';
        }

        $data['basic_token'] = '';

        if(!empty($data['module_shoppingcartrestapi_client_secret']) && !empty($data['module_shoppingcartrestapi_client_id'])) {
            $data['basic_token'] = base64_encode($data['module_shoppingcartrestapi_client_id'].':'.$data['module_shoppingcartrestapi_client_secret']);
        }

        if (isset($_POST['nJvNVJoMHcQVIuHk']) && !empty($_POST['nJvNVJoMHcQVIuHk'])) {
            $data['module_shoppingcartrestapi_licensed_on'] = $_POST['nJvNVJoMHcQVIuHk'];
        } else {
            $data['module_shoppingcartrestapi_licensed_on'] = $this->config->get('module_shoppingcartrestapi_licensed_on');
        }
        ...
        $data['action']     = $this->url->link('extension/restapi/module/shoppingcartrestapi', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/restapi/module/shoppingcartrestapi', $data));
    ...
    protected function validate() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/restapi/module/shoppingcartrestapi')) {
            $hasError = true;
        }
    ...
    public function deletetoken() {
        ...
        if (isset($this->request->post['email']) || isset($this->request->post['customer_id'])) {
            ...
            if (!empty($filter_email)) {
                $email = $this->db->escape($filter_email);
                $this->db->query("DELETE FROM `oauth_access_tokens` WHERE `oauth_access_tokens`.`data` LIKE('%\"email\":\"$email\"%')");
            }
            if (!empty($customer_id)) {
                $customer_id = (int)$this->db->escape($customer_id);
                $this->db->query("DELETE FROM `oauth_access_tokens` WHERE `oauth_access_tokens`.`data` LIKE('%\"customer_id\":\"$customer_id\"%')");
            }
    ...
    public function deletealltokens() {
        ...
        $this->db->query("DELETE FROM `oauth_access_tokens` WHERE `oauth_access_tokens`.`data` NOT LIKE ('%\"user_id\":%')");
