...
class Vendor extends \Opencart\System\Engine\Model {
	...
	$this->db->query("INSERT INTO " .  DB_PREFIX . "multivendor SET display_name = '" . $this->db->escape($data['display_name']) . "',firstname = '" . $this->db->escape($data['firstname']) . "',lastname = '" . $this->db->escape($data['lastname']) . "',email = '" . $this->db->escape($data['email']) . "',telephone = '" . $this->db->escape($data['telephone']) . "',fax = '" . $this->db->escape($data['fax']) . "',company = '" . $this->db->escape($data['company']) . "',address_1 = '" . $this->db->escape($data['address_1']) . "',address_2 = '" . $this->db->escape($data['address_2']) . "',map_url = '" . $this->db->escape($data['map_url']) . "',city = '" . $this->db->escape($data['city']) . "',country_id = '" .(int)$data['country_id'] . "',zone_id = '" .(int)$data['zone_id'] . "', `password` = '" . $this->db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "',status = '1',approved = '".$autoapprovedseller."',about = '" . $this->db->escape($data['about']) . "',image='".$data['image']."',logo='".$data['logo']."',store_about='".$this->db->escape($data['store_about'])."',bank_detail='".$this->db->escape($data['bank_detail'])."',banner='".$data['banner']."',payment_method='".$this->db->escape($data['payment_method'])."',paypal = '" . $this->db->escape($data['paypal']) . "', bank_name = '" . $this->db->escape($data['bank_name']) . "', bank_branch_number = '" . $this->db->escape($data['bank_branch_number']) . "', bank_swift_code = '" . $this->db->escape($data['bank_swift_code']) . "', bank_account_name = '" . $this->db->escape($data['bank_account_name']) . "', bank_account_number = '" . $this->db->escape($data['bank_account_number']) . "',tax_number='".$this->db->escape($data['tax_number'])."',shipping_charge='".$this->db->escape($data['shipping_charge'])."',date_added = NOW(), postcode = '" .$this->db->escape($data['postcode']). "'");
		...
		if(!empty($mailinfo['status'])){
			...
			$replace = array(
				...
				'zonename' 		=> $zonename,
				'city' 		    => $data['city'],
				'loginlink' 	=> $this->url->link('extension/tmdmultivendor/vendor/login', '', true) . "\n\n"
			);
		...
		// Send to main admin email if new vendor email is enabled		
			$this->load->language('extension/tmdmultivendor/vendor/seller_order');
	...
	public function editPassword(int $vendor_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "vendor` SET `password` = '" . $this->db->escape(password_hash(html_entity_decode($data['password'], ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "',date_modified=now() WHERE vendor_id = '" . (int)$vendor_id ."'");
	}
...
// Seller Review Start ///	
	...
	public function addReview($data,$vendor_id){
		$this->db->query("INSERT INTO " .  DB_PREFIX . "vendor_review set text='".$this->db->escape($data['text'])."',vendor_id='".(int)$vendor_id."',customer_id='".(int)$this->customer->getId()."',status=1,date_added=now()");
	...
	public function addTracks($order_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "vendor_order_product set
	...
	public function addOrdeStatus($order_id,$data,$vendor_id) {
		...
		$sellerorder_info = $this->model_extension_tmdmultivendor_vendor_vendor->getSellerOrder($order_id);
		$seller_info = $this->model_extension_tmdmultivendor_vendor_vendor->getVendor($sellerorder_info['vendor_id']);
		...
		$mailinfoadmin = $this->model_extension_tmdmultivendor_vendor_mail->getMailInfo($sellertypeadmin);
	...
	public function getTotalProducts($vendor_id){
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "vendor_to_product v2p LEFT JOIN ".DB_PREFIX."product p ON(v2p.product_id = p.product_id) WHERE v2p.vendor_id='".(int)$vendor_id."'";
	...
	public function getTotalOrderProductsByOrderId($order_id, $vendor_id) {
		...
		$query = $this->db->query("SELECT sum(quantity) AS total FROM " . DB_PREFIX . "vendor_order_product WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . (int)$vendor_id . "'");
	...
	public function getvendorOrdertotal($vendor_id, $order_id) { 
		...
		$sql="SELECT SUM(total) AS total,SUM(tmdshippingcost) AS shipingcost,sum(tax*quantity) as tax FROM " . DB_PREFIX . "vendor_order_product WHERE vendor_id='".(int)$vendor_id."' AND order_id='".(int)$order_id."'";		

		$query = $this->db->query($sql);	

		$query->row['total'] += $query->row['tax']+$query->row['shipingcost'];
		return $query->row;
	...
	public function getTotalDashboardReviews($vendor_id){
