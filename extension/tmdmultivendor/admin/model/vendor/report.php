...
class Report extends \Opencart\System\Engine\Model {
	...
	public function addTracks($order_product_id, $data) {
		$this->db->query("UPDATE " .  DB_PREFIX . "vendor_order_product set	tracking='".(int)$data['tracking']."',date_modified=now() WHERE order_product_id='".(int)$order_product_id."'");
	}
	
	public function addOrdeStatus($order_id,$data) {
		$this->db->query("UPDATE " .  DB_PREFIX . "vendor_order_product set	order_status_id='".(int)$data['order_status_id']."',date_modified=now() where order_id='".(int)$order_id."'");
	...
	public  function getMailInfo($sellertype){
 	...
 	public function deleteReport($order_product_id){		
		$this->db->query("DELETE FROM " . DB_PREFIX . "vendor_order_product where order_product_id='".(int)$order_product_id."'");
 	} 
	...
	public function getOrderTotals($order_id){
 	...
 	public function getReports($data){		
		...
		if (!empty($data['filter_date'])) {
			$sql .= " AND DATE(vop.`date_added`) = DATE('" . $this->db->escape((string)$data['filter_date']) . "')";
		}
	...
	public function getVendorName($order_product_id) {
		$query = $this->db->query("SELECT  * FROM " . DB_PREFIX . "vendor_order_product vop LEFT JOIN ".DB_PREFIX."multivendor v on vop.vendor_id = v.vendor_id WHERE vop.order_product_id = '" . (int)$order_product_id . "'");
		return $query->row;
	}
