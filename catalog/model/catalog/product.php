...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProductsAllData($data , $customer) {
	...
	public function getProductsTotal($data, $customer, $onlycount=true) {
   ...
   public function getProductsData($data , $customer) {
		if ($customer->isLogged()) {
			$customer_group_id = $customer->getGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	private function searchHelper($request){
		...
		foreach($request['filters'] as $filter){
			if(array_key_exists($filter['field'], static::$fields)) {
				$filter['operand'] = html_entity_decode($filter['operand']);
	...
	public function search($data, $request, $customer) {
		if ($customer->isLogged()) {
			$customer_group_id = $customer->getGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
