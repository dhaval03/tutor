...
class Attribute extends \Opencart\System\Engine\Model {
	public function addAttribute($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_to_attribute SET attribute_id = '" . (int)$attribute_id . "', vendor_id = '" . (int)$this->vendor->getId() . "'");
	...
	public function getAttributes($data = array()) {
		...
		if(isset($data['vendor_id'])){
			$sql .= " and v2a.vendor_id='".(int)$data['vendor_id']."'";
		}
		...
		if (!empty($data['filter_attribute_group_id'])) {
			$sql .= " AND a.attribute_group_id = '" . (int)$this->db->escape($data['filter_attribute_group_id']) . "'";
		}
