...
public function index(): void {	
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'extension/tmdmultivendor/vendor/import')) {
			...
			$model	=	$xmlData->MODEL;
				...
				if(isset($catgorynames))
				{
					foreach($catgorynames as $category)
					{
						...
						if(!empty($category))
						{
							$category_id[]=$this->model_extension_tmdmultivendor_vendor_import->category($category,0,$store_id,$language_id);
						}
					}
				}
				/* Step Get category ids  */
				...
				if(isset($options))
				{
					foreach($options as $option)
					{
						...
						if(!empty($option))
						{
							$this->model_extension_tmdmultivendor_vendor_import->option($option,$language_id);
						}
					}
				}
				...
				$product_id=$this->model_extension_tmdmultivendor_vendor_import->getproductbymodel($model,$vendor_id);
				if(empty($product_id))
				{
					$this->model_extension_tmdmultivendor_vendor_import->addproduct($this->data,$language_id,$extra,$vendor_id);
					$totalnewproduct++;
				}
				else
				{
					$this->model_extension_tmdmultivendor_vendor_import->editproduct($this->data,$product_id,$language_id,$extra,$vendor_id);
					$totalupdateproduct++;
				}
				/* Step Product other  info collect */
				//print_r($xmlData);
				...
				////////////////////////// Started Import work  //////////////
				$this->response->redirect($this->url->link('extension/tmdmultivendor/vendor/import', 'user_token=' . $this->session->data['user_token']));
			}
			if ($content && $this->request->post['format']=='csv')
			{
				...
				*/
				...
				if(isset($product_stornew))
				{
					...
					if(!empty($product_store))
						{
							 $product_store=$this->model_extension_tmdmultivendor_vendor_import->getstorebyname($product_store,$vendor_id);
							$product_stores[]=$product_store;
						}
					}
				}
				...
				foreach($relatedprodctinfos as $relatedprodctinfo)
					...
					if(!empty($relatedprodctinfo))
						{
						$product_related[]=$this->model_extension_tmdmultivendor_tmd_import->getproductbymodel($relatedprodctinfo,$vendor_id);
					}
					}
				}
			...
			else if ($content)
			{
				...
				*/
				...
				////////////////////////// Started Import work  //////////////
				$this->response->redirect($this->url->link('extension/tmdmultivendor/vendor/import', 'user_token=' . $this->session->data['user_token'], true));
			} 
			 else {
		...
		$data['import'] = $this->url->link('extension/tmdmultivendor/vendor/import', 'user_token=' . $this->session->data['user_token']);
