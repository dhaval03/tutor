...
class Notification extends \Opencart\System\Engine\Model {
	...
	public function addNotification(array $data): int {
		$this->db->query("INSERT INTO "  . DB_PREFIX . "vendor_notification set type='".$data['type']."',date='".$this->db->escape($data['date'])."',date_added=now()");
	...
	public function editNotification(int $notification_id, array $data): void {
		$this->db->query("UPDATE " . DB_PREFIX . "vendor_notification set type='".$data['type']."',date='".$this->db->escape($data['date'])."',date_modified=now() where notification_id = '" . (int)$notification_id . "'");
