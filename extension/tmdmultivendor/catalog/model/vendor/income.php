...
class Income extends \Opencart\System\Engine\Model {
	...
	public function addAmount($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_amount_pay set vendor_id='".(int)$data['vendor_id']."',payment_method='".$this->db->escape($data['payment_method'])."',amount='".(float)$data['amount']."',comment='".$this->db->escape($data['comment'])."',date_added=now()");
	}
	...
	public function getAmount($vendor_id){
 	...
 	public function getIncomes($data=array()){
		...
		if (!empty($data['filter_date_from'])) {
			$sql .= " AND DATE(date_added) >= '" . $data['filter_date_from'] . "'";
		}
